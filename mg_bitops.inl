...
template <typename t>
mg_ForceInline t SetBit(t Val, int I) {
...
template <typename t>
mg_ForceInline t UnsetBit(t Val, int I) {
  mg_Assert(I < mg_BitSizeOf(t));
  return Val & t(~(1ull << I));
}
...
template <typename t>
mg_ForceInline bool CheckBit(t Val, int I) {
  mg_Assert(I < mg_BitSizeOf(t));
  return 1 & (Val >> I);
}
...
template <typename t>
mg_ForceInline t FlipBit(t Val, int I) {
  mg_Assert(I < mg_BitSizeOf(t));
  return Val ^ t(1ull << I);
}
...
mg_ForceInline i8 BitScanReverse(u32 V) {
  if (V == 0) return -1;
  return i8(mg_BitSizeOf(V) - 1 - __builtin_clz(V));
}
mg_ForceInline i8 BitScanReverse(u64 V) {
  if (V == 0) return -1;
