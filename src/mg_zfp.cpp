...
bool
Encode(u64* Block, int B, int S, i8& N, i8& M, bool& In, bitstream* Bs) {
  mg_Assert(N <= 64);
  u64 X = 0;
  for (int I = M; I < 64; ++I)
...
bool
Decode(u64* Block, int B, int S, i8& N, i8& M, bool& In, bitstream* Bs) {
  i8 P = Min(N - M, S - (int)BitSize(*Bs));
...
void
Encode32(u32* Block, int B, int S, i8& N, bitstream* Bs) {
  mg_Assert(N <= 64);
  u64 X = 0;
  for (int I = 0; I < 64; ++I)
    X += u64((Block[I] >> B) & 1u) << I;
...
void
Decode32(u32* Block, int B, int S, i8& N, bitstream* Bs) {
  i8 P = Min((int)N, S - (int)BitSize(*Bs));
  u64 X = P > 0 ? ReadLong(Bs, P) : 0;
  for (; BitSize(*Bs) < S && N < 64;) {
    if (Read(Bs)) {
